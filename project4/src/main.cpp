...
void set_waypoints()
{
    ...
    int startQuad;	  //the quadrant of the starting point
    point temp_candid[4]; //point to save the temp_candid points
    ...
    //I'll implement the problem dividing the map into 4
    //First, we need to check which quadrant is the car starting at
    if(waypoint_candid[0].x < 0){
        if(waypoint_candid[0].y < 0){ startQuad = 3;}
	else { startQuad = 2;}
    }else{
        if(waypoint_candid[0].y < 0){ startQuad = 4;}
	else { startQuad = 1;}
    }
    printf("startQuad = %d\n",startQuad);

    //random sample the 3 points.
    for(int i = 1; i <= 4; ++i){
        if(i != startQuad){
            //random sample point until the point color is white
	    do{
		    temp_candid[i-1].x = world_x_max * ((double)rand()/(double)RAND_MAX);
		    temp_candid[i-1].y = world_y_max * ((double)rand()/(double)RAND_MAX);
		    //printf("temp_candid[%d].x = %.2f, .y = %.2f\n",i-1,temp_candid[i-1].x,temp_candid[i-1].y);
		    //printf("color = %d\n",map_margin.at<uchar>(map_origin_x + temp_candid[i-1].x/res,map_origin_y + temp_candid[i-1].y/res));
    	    }while(map_margin.at<uchar>(map_origin_x + temp_candid[i-1].x/res,map_origin_y + temp_candid[i-1].y/res) == 0);

	    //negate the coordinate according to its quadrant
            switch(i){
                case 1: 
		    break;
		case 2:
	...
	}else{ temp_candid[startQuad-1] = waypoint_candid[0]; }
	printf("temp_candid[%d].x = %.2f, .y = %.2f\n",i-1,temp_candid[i-1].x, temp_candid[i-1].y);
    }

    for(int i = 1; i < 4; ++i){
        waypoint_candid[i] = temp_candid[(startQuad - i + 3) % 4];
	printf("waypoint_candid[%d] = (%.2f,%.2f)\n",i,waypoint_candid[i].x,waypoint_candid[i].y);
    }
