...
float distance_check_arrive = 0.2;
...
int main(int argc, char** argv){
    ...
    float distance, distance_old, x_err, y_err;
    ...
    while(ros::ok()){
        ...
        */
        ...
        if (drive_msg_stamped.drive.steering_angle > 0) {
            drive_msg_stamped.drive.steering_angle = MIN(drive_msg_stamped.drive.steering_angle, max_turn); 
        }
        else {
            drive_msg_stamped.drive.steering_angle = MAX(drive_msg_stamped.drive.steering_angle, -max_turn); 
        }

        x_err = path[current_goal].x - car_pose.x;
        y_err = path[current_goal].y - car_pose.y;
        distance = pow(pow(x_err, 2) + pow(y_err, 2), 0.5);
        
        x_err = path[current_goal-1].x - car_pose.x;
        y_err = path[current_goal-1].y - car_pose.y;
        distance_old = pow(pow(x_err, 2) + pow(y_err, 2), 0.5);
        ...
        else if ( distance < distance_check_arrive*3 || distance_old < distance_check_arrive*3) {
            //drive_msg_stamped.drive.speed = MIN(max_speed, drive_msg_stamped.drive.speed + 0.1);
            drive_msg_stamped.drive.speed += 0.1 / MAX(1, abs(ctrl));//0.5*MAX(0.05, 0.002 / abs(ctrl));
            if (drive_msg_stamped.drive.speed > max_speed) {
                drive_msg_stamped.drive.speed = max_speed;
            }
        }
        else {
            //drive_msg_stamped.drive.speed = MIN(max_speed, drive_msg_stamped.drive.speed + 0.15);
            drive_msg_stamped.drive.speed += 0.2 / MAX(0.05, abs(ctrl));
            if (drive_msg_stamped.drive.speed > max_speed) {
                drive_msg_stamped.drive.speed = max_speed;
            }
        }
